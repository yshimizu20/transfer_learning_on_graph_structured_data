...
def train_product():
    ...
    data_product = data_product.to(device)
    ...
    for run in range(3):
        ...
        for epoch in range(1, 1 + epochs):
            loss = train(model, data_product, train_idx_product, optimizer)

            if epoch % eval_steps == 0:
                result = test(model, data_product, split_idx_product, evaluator)
...
def train_protein():
    ...
    for run in range(3):
        ...
        optimizer = torch.optim.Adam(model.parameters(), lr=lr)
        for epoch in range(1, 1 + epochs):
            loss = train(model, data_protein, train_idx_protein, optimizer)

            if epoch % eval_steps == 0:
                result = test(model, data_protein, split_idx_protein, evaluator)
                logger.add_result(run, result)
...
def transfer_protein_to_product():
    ...
    pretrained_model = SAGE(
        data_protein.num_features, hidden_channels, 47, num_layers, dropout
    ).to(device)

    pretrained_model.load_state_dict(torch.load("gnn_proteins.pt"))
    ...
    for run in range(3):
        ...
        for param, pretrained_param in zip(
            ...
            param.data = pretrained_param.data.clone()
...
def transfer_product_to_protein():
    ...
    pretrained_model.load_state_dict(torch.load("gnn_products.pt"))
